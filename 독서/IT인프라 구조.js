...
3.2 3계층형 시스템의 주요 개념 설명
  ...
  ㄴ.프로세스 관리 - 가동되고 있는 프로세스 관리와 CPU 이용 우선순위 등을 '스케줄' 한다.
...
4.6 가변길이/고정 길이
  4.6.2 어디에 사용되나?
  *PO에서 JSON메시지는 가변길이. fixedlength는 패킷전문 보낼 때
  컴퓨터에서는 다양한 파일이 저장돼 있다. 파일들 오른쪽 눌러서 보면 고정길이로 저장돼어 있다.
  네트워크에서 데이터를 교환할 때는 : TCP/IP는 보통 가변길이를 사용함 헤더 부분 40byte 나머지는 데이터 부분.
  가변길이는 데이터 전체 양이 줄기 때문에 효율성이 좋다
  고정 길이는 크기가 균일해서 관리가 수월하다. 둘 중 적절하게 선별해서 사용할 것.

4.7 데이터 구조(배열과 연결 리스트)
  4.7.1 데이터 구조란?
  배열은 같은 형태의 상자를 빈틈없이 순서대로 나열한 데이터 구조다. '같은 크기의 상자를' '빈틈없이 순서대로' 나열하기 때문에
  몇 번째 상자인지만 알면 해당 상자에 한 번에 액세스할 수 있다.
  연결리스트는 상자를 선으로 연결한 형태의 데이터 구조다. 다음 상자의 위치 정보를 가지고 있다. ]

  배열의 단점은 데이터를 추가하려면 나머지 전체를 옮겨야 한다. 뺄 때도 전체를 앞으로 옮겨야 한다. 이런 이유로 배열은 데이터 추가
  및 삭제가 느린 데이터 구조라고 할 수 있다.

  4.7.2 어디에 사용되나?

7.1 안정성 및 이중화
  7.1.2 이중화란?
  간단히 말해서 하나의 기능을 병렬로 여러 개 나열해서 하나에 장애가 발생해도 다른 것을 이용해서 서비스를 계속할 수 있는 것을 말함.
