...
exports.search_author = function(request, response){
    ...
    db.query(`SELECT * FROM topic`, function(error,topics){ //여기서 function은 쿼리문이 실행된후에 실행될 콜백 함수임
        //db.query(`SELECT * FROM author where name =?`,[post.searchAuthorName],function(error2,authors){
        //db.query(`SELECT * FROM author where name Like '%t%'`,function(error2,authors){
          console.log("*****************************************");
          console.log(authors.length)
...
exports.search_author_process = function(request, response){
    var body = '';
      request.on('data', function(data){
          body = body + data;
          console.log("-----------/author/search_author_process log------------------");
          console.log(body); //화면에서 입력한 값.
          console.log("-----------/author/search_author_process log------------------");
      });
      request.on('end', function(){
          var post = qs.parse(body);
          console.log("-----------222/author/search_author_process log------------------");
          console.log(post);
          console.log("-----------222/author/search_author_process log------------------");
          db.query(`
            SELECT * FROM author where name=?`,
            [post.searchAuthorName],
            function(error, result){
              if(error){
                throw error;
              }
              response.writeHead(302, {Location: `/search_author`});
              //200은 성공, 302는 다른페이지로 리다이렉션시키라는 뜻
              //create를 한 다음에 그 생성한 페이지로 이동하게끔!
              response.end();
